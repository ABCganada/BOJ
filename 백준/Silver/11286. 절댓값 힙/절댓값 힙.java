...
public class Main {
    ...
    private static void upHeap(int i){
        ...
        while(i > 1 && Math.abs(arr[i]) <= Math.abs(arr[i/2])){
    ...
    private static void downHeap(int i){
        ...
        while(2*i <= lastIdx){
            ...
            if(rightChild <= lastIdx){
                if(Math.abs(arr[leftChild]) == Math.abs(arr[rightChild])){
                    largestChild = arr[leftChild] < arr[rightChild] ? leftChild : rightChild;
                } else {
                    largestChild = Math.abs(arr[leftChild]) < Math.abs(arr[rightChild]) ? leftChild : rightChild;
                }
            }

            if(Math.abs(arr[i]) > Math.abs(arr[largestChild])){
                swap(i, largestChild);
                i = largestChild;
            } else if(Math.abs(arr[i]) == Math.abs(arr[largestChild]) && arr[i] > arr[largestChild]){
                swap(i, largestChild);
                i = largestChild;
            }else{
                break;
